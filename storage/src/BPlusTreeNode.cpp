...
bool BPlusTreeNode::load(std::uint32_t _addr) {
    ...
    // 读入元素数量
    pager.read(currentAddr, (char *) &(len), sizeof(len));
    ...
    // 读入 nextLeaf
    pager.read(currentAddr, (char *) &(nextLeaf), sizeof(nextLeaf));
...
bool BPlusTreeNode::dump(std::uint32_t _addr) {
    ...
    // 写入 key 列表
    pager.write((char *) keys.data(), sizeof(keys[ 0 ]) * order, currentAddr);
    currentAddr += sizeof(keys[ 0 ]) * order;

    // 写入 childrenOrValue 列表
    pager.write((char *) childrenOrValue.data(),
                sizeof(childrenOrValue[ 0 ]) * order, currentAddr);
    currentAddr += sizeof(childrenOrValue[ 0 ]) * order;
