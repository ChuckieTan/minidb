...
public:
    ...
    std::uint64_t read_journal_file(void *data, std::uint64_t size,
                                    std::uint64_t pos);

    std::uint64_t write_journal_file(const void *data, std::uint64_t size,
                                     std::uint64_t pos);

    std::uint64_t new_page();

    std::uint64_t get_index_file_size();
    std::uint64_t get_data_file_size();
    std::uint64_t get_journal_file_size();

    bool is_in_memory;

    std::fstream index_file;
    std::fstream data_file;
    std::fstream journal_file;
};
} // namespace minidb::storage